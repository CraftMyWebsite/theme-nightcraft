...
<section class="mb-8 px-8 md:px-36 2xl:px-96">
    <div class="lg:grid grid-cols-4 gap-6">
            ...
            <h4 style="color: var(--main-color)" class="text-center"><?= ThemeModel::getInstance()->fetchConfigValue('wiki_menu_title') ?></h4>
                ...
                <div class="flex flex-col space-y-1">
                    <p class="uppercase font-bold">
                        <?php if(ThemeModel::getInstance()->fetchConfigValue('wiki_display_categorie_icon')): ?><i class="<?= $categorie->getIcon() ?>"></i><?php endif; ?> <?= $categorie->getName() ?></p>
                    <?php foreach ($categorie?->getArticles() as $menuArticle): ?>
                    <a href="<?=   EnvManager::getInstance()->getValue("PATH_SUBFOLDER") . "wiki/" . $categorie->getSlug() . "/" . $menuArticle->getSlug() ?>" class="a-base ml-3"><?php if(ThemeModel::getInstance()->fetchConfigValue('wiki_display_article_categorie_icon')): ?><i class="<?= $menuArticle->getIcon() ?>"></i><?php endif; ?> <?= $menuArticle->getTitle() ?></a>
                        <?php endforeach; ?>
                </div>
                    <?php endforeach; ?>
        </div>
        <div style="background: var(--card-bg-color);" class="rounded-lg p-6 col-span-3 h-fit">
            ...
            <hr>
            <div class="flex flex-wrap justify-between items-center mt-2">
                <?php if(ThemeModel::getInstance()->fetchConfigValue('wiki_display_creation_date')): ?><p class="text-xs">Crée le : Jamais</p><?php endif; ?>
                <?php if(ThemeModel::getInstance()->fetchConfigValue('wiki_display_autor')): ?><p class="text-xs py-1 px-2 bg-gray-300">Personne</p><?php endif; ?>
